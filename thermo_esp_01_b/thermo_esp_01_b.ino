...
void CheckForClients(){
  if(Server.hasClient()){
    if(RemoteClient.connected())
    {
      ...
      RemoteClient.write("Connected to SSID: ");
      RemoteClient.write(ssid);
      RemoteClient.write('\n');
      RemoteClient.write("Local IP: ");
      char ip[40];
      char oct[4];
      char d[1] = {'.'};
      char str[16];
      for(int i = 0; i < sizeof(WiFi.localIP()); ++i){
        snprintf(oct, sizeof(oct), "%i", (int)WiFi.localIP()[i]);
        if(i == 0){
          for(int j = 0; j < sizeof(oct); ++j){
            ip[j] = oct[j];
          }
        }
        else
          strcat(ip, oct);
        yield();
        if(i < sizeof(WiFi.localIP())-1)
          strcat(ip, d);
      }
      RemoteClient.write(ip, 13);
      RemoteClient.write('\n');
      RemoteClient.write("Port: ");
...
int EchoReceivedData(uint8_t *receiveBuffer, size_t s)
{
  int received = 0;
...
void loop() {
  ...
  if(int(Serial.read()) == 84){
    ...
    int rec = EchoReceivedData(receiveBuffer, sizeof(receiveBuffer));
    if(rec > 0 && rec <=5){
      for(int i = 0; i < rec; ++i){
      ...
      temp = getTemp(byteBuffer, dcount, counter);
      if(RemoteClient.connected()){
        RemoteClient.write("I found: ");
        RemoteClient.write(receiveBuffer, rec);
        RemoteClient.write("I set: ");
        char str[16];
